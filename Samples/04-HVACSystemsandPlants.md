...
3. Simple Leaving "system" water temperature and Leaving "system water temperature setpoint models should be used on systems rather than specifying between primary/secondary/tertiary. Primary/secondary/tertiary are communicated in the twin "NAME" only. For Heat Exchangers, the primary/secondary designation is represented by a HeatExchangerPrimarySide/HeatExchangerSecondarySide component (isPartOf -> HeatExchanger) rather than by a naming prefix on the exchanger's own points. For Heat Exchanger Groups the same pattern applies through a parallel model chain - HeatExchangerGroupPrimarySide/HeatExchangerGroupSecondarySide (isPartOf -> HVACHeatExchangerGroup) - because AssetComponent.isPartOf (which the HeatExchanger-side models extend through) can only target an Asset, and HVACHeatExchangerGroup is a Collection. Each side component, on either chain, carries a `medium` property recording its fluid (ChilledWater, CondenserWater, HotWater, Steam, SteamCondensate) plus its own design values (design entering/leaving/delta water temperature, design flow). Entering water temperature, leaving water temperature, delta T, water delta pressure, and water flow points are modeled as capabilities of the side component using the neutral (non-medium-specific) sensor models - not directly on the Heat Exchanger or Heat Exchanger Group, and not using medium-prefixed sensor models.
